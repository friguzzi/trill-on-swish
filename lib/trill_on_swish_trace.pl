...
:- use_module(trill_on_swish_storage).
...
trill_on_swish_no_lco.

:- '$hide'(trill_on_swish_call/1).
:- '$hide'(trill_on_swish_no_lco/0).
...
set_file_breakpoints(_Pengine, PFile, Text, Dict) :-
	...
	->  debug(trace(break), 'Pengine included source ~p', [StoreFile]),
	    trill_on_swish_storage_file(StoreFile, IncludedText, _Meta),
...
add_breakpoint(PFile, File, _Text, Line) :-
	...
	trill_on_swish_storage_file(Store, Text, _Meta),
...
prolog_clause:open_source(File, Stream) :-
	...
	trill_on_swish_storage_file(GittyFile, Data, _Meta),
