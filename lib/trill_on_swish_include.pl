...
trill_on_swish:term_expansion(:- include(FileIn), Expansion) :-
	...
	;   Expansion = [ (:- discontiguous('swish included'/1)),
	    ...
	    catch(trill_on_swish_gitty_data(Store, FileExt, Data, _Meta), _, fail),
...
prolog_colour:term_colours((:- include(File)),
	...
	(   atomic(File),
	    setting(trill_on_swish_web_storage:directory, Store),
	    add_extension(File, FileExt),
	    catch(trill_on_swish_gitty_commit(Store, FileExt, _Meta), _, fail)
	->  atom_concat('swish://', FileExt, Id),
	    FileClass = file(Id)
	;   FileClass = nofile
	),
...
prolog:xref_open_source(File, Stream) :-
	...
	setting(trill_on_swish_web_storage:directory, Store),
	catch(trill_on_swish_gitty_data(Store, Name, Data, _Meta), _, fail),
	open_string(Data, Stream).
