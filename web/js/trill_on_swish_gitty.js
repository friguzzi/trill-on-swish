/**
 * @fileOverview
 * Dialog components to interact with the trill_on_swish_gitty store.
 *
 * @version 0.2.0
...
(function($) {
  var pluginName = 'trill_on_swish_gitty';

  /** @lends $.fn.trill_on_swish_gitty */
  var methods = {
    /**
     * @param {Object} options
     * @param {Object.meta} provides the trill_on_swish_gitty meta-data
     */
	...
	elem.append($.el.ul(
	  {class:"nav nav-tabs"},
	  tab("Meta data", true,  "trill_on_swish_gitty-meta-data"),
	  tab("History",   false, "trill_on_swish_gitty-history",  henabled),
	  tab("Changes",   false, "trill_on_swish_gitty-diff",     henabled)));
	elem.append(tabs);

	/* meta-data tab */
	tabs.append($.el.div({ class:"tab-pane fade in active trill_on_swish_gitty-meta-data",
	                       id:"trill_on_swish_gitty-meta-data"}));
	elem.find('[href="#trill_on_swish_gitty-meta-data"]').on("show.bs.tab", function(ev) {
	  elem.trill_on_swish_gitty('showMetaData');
	});

	/* history tab */
	tabs.append($.el.div({ class:"tab-pane fade trill_on_swish_gitty-history",
	                       id:"trill_on_swish_gitty-history"}));
	elem.find('[href="#trill_on_swish_gitty-history"]').on("show.bs.tab", function(ev) {
	  elem.trill_on_swish_gitty('showHistory');
	});

	/* diff/changes tab */
	tabs.append($.el.div({ class:"tab-pane fade trill_on_swish_gitty-diff",
	                       id:"trill_on_swish_gitty-diff"}));
	elem.find('[href="#trill_on_swish_gitty-diff"]').on("show.bs.tab", function(ev) {
	  elem.trill_on_swish_gitty('showDiff');
	});
    ...
    /**
     * @param is the trill_on_swish_gitty meta-object
	...
	var elem = $(this);
	var data = elem.data(pluginName);
	var tab  = elem.find(".trill_on_swish_gitty-meta-data");
	var formel;
	...
	var elem = $(this);
	var data = elem.data(pluginName);
	var tab  = elem.find(".trill_on_swish_gitty-history");
	var meta = data.commits[data.commit];
	var playButton;
	...
	tab.html("");
	tab.append($.el.table(
	  { class:"table table-striped table-condensed trill_on_swish_gitty-history",
	...
	$.ajax({ url: url,
		 ...
		 success: function(reply) {
		   elem.trill_on_swish_gitty('fillHistoryTable', reply);
    ...
    fillHistoryTable: function(history) {
      var trill_on_swish_gitty = this;
      var data  = this.data(pluginName);
      var table = this.find(".table.trill_on_swish_gitty-history");
	...
	trill_on_swish_gitty.trill_on_swish_gitty('setCommit', commit);
      });
    },
    ...
    setCommit: function(commit) {
      ...
      h2.html("");
      h2.append(this.trill_on_swish_gitty('title', data.commits[commit]));
	...
	$.ajax({ url: url,
		 ...
		 success: function(reply) {
		   elem.trill_on_swish_gitty('fillDiff', reply);
    ...
    fillDiff: function(diff) {
      if ( diff.tags ) this.trill_on_swish_gitty('diffTags', diff.tags);
      if ( diff.data ) this.trill_on_swish_gitty('udiffData', diff.data);
    },

    diffTags: function(diff) {
      var tab  = this.find(".trill_on_swish_gitty-diff");
  ...
  $.fn.trill_on_swish_gitty = function(method) {
    if ( methods[method] ) {
